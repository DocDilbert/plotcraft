 ...
 public:
  ...
  std::vector<double> GetTickValues(double vmin, double vmax) {
    ...
    for (unsigned i = 0; i < num_ticks_; i++) {
      double val =
          (vmax - vmin) * static_cast<double>(i) / static_cast<double>(num_ticks_ - 1) + vmin;
      ret.push_back(val);
    }
    return ret;
  }
 ...
 public:
  ...
  std::vector<double> GetTickValues(double vmin, double vmax) {
    ...
    for (auto it : nice_values) {
      ...
      // 10
      double step = pow(10.0, static_cast<int>(std::floor(std::log10(raw_steps)))) * it;

      int lower_limit = static_cast<int>(std::ceil(vmin / step));
      int upper_limit = static_cast<int>(std::floor(vmax / step));
